...
console.log(arr4)//분리할 게 없을 경우 단일배열

arr4 = insa.split("");//한 문자씩 분리
console.log(arr4)//문자열 한 자 한 자 분리
...
//Q.문자열형태의 배열을 입력받을 경우(arr7) 이 데이터를
//배열객체로 변경하시오.
...
//즉 json 형식은 key와 value를 ""으로 묶어야 가능
...
//for문은 기본 for(초기값;제어값;증가값;), foreach, for( in ), for( of )
